...
module.exports = (indexKeys, queries, schema, overrides) => {
  ...
  const printIndexResults = (results) => {
    const data = results.map(({ time, docsExamined, keysExamined, numberOfResults, id }, i) => (
      { 'Rank': chalk.yellow(i + 1), 'Index': id, 'Time (ms)' : time, 'Documents examined': docsExamined, 'Keys examined': keysExamined, 'Number of results': numberOfResults }
    ));
    console.log();
